# compactor docs

Start here:

- [Install](./install.md): current source install and verification path.
- [Getting started](./getting-started.md): first successful local workflow.
- [Usage](./usage.md): current and planned command surface.
- [Architecture](./architecture.md): product model, boundaries, and early design questions.
- [Release](./release.md): release expectations once packaging exists.
- [Contributing](./contributing.md): development loop and repo conventions.
- [Agent context](./agents/README.md): progressive-disclosure map for coding agents.

The repository is still in planning scaffold state. Prefer updating these docs as product decisions become concrete instead of letting assumptions live only in chat history.

